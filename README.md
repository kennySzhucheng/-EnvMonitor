...
这个项目最大的收获不是做出来一个东西，而是 **5 天里从零啃下了 4 块硬骨头**：

| 技能 | 项目前 | 项目后 |
|------|--------|--------|
| **STM32 开发方式** | 只会标准库，没碰过 HAL | CubeMX 配置 + HAL 库驱动开发，理解句柄/初始化回调 |
| **FreeRTOS** | 完全不了解 | 6 任务抢占式调度，互斥量/队列/软件定时器，SysTick 三分天下 |
| **PCB 设计** | 零基础，没画过板 | KiCad 10.0 原理图→封装→布局→布线→铺铜→DRC→Gerber 全流程 |
| **项目工程化** | 单文件裸机循环 | 三层架构(Drivers/App/Core)，Git 版本控制，README 文档规范 |

> 💡 KiCad 入门借助了 Claude Code 的 KiCad Skill 辅助分析，但具体画板——元件摆放、走线、铺铜——还是得自己动手一刀一刀画，其实我以为可以让ai帮我解决排线问题，实则搞半天还是得自己来而且第一次画确实特别乱，没有经验，更改了很多遍
